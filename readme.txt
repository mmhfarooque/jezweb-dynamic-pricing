...
* FIXED: Textdomain loading too early notice in WordPress 6.7+
* Improved textdomain loading order for translations
* Added debug tracing for get_applicable_rules() and calculate_price()
...
* Added special_offer_type to database - this was the root cause of settings not persisting
* Special offer dropdown now correctly restores saved selection when editing rules
* Database migration automatically adds the new column

= 1.5.9 =
* Debug release: Added logging for INSERT operations (new rules)
* Added logging for rule LOAD operations
* Verifies event_type in database immediately after INSERT/UPDATE
* Full trace of save and load cycle
